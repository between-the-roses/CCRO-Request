...
<head>
  ...
  <title>Marriage Certificate Form - Request for Others</title>
    ...
    <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="confirmModalLabel">Are you requesting for the same person?</h5>
  ...
  <script>
    ...
    function proceedToNextStep() {
      ...
      if (anyIdSelected) {
        ...
        // Submit the form
        document.getElementById('marriageCertForm').submit();
      } else {
    ...
    function handleYes() {
      ...
      modal.hide();
      addSamePersonCertificate();
    }

    function handleNo() {
      const modal = bootstrap.Modal.getInstance(document.getElementById('confirmModal'));
      modal.hide();
      addDifferentPersonCertificate();
    }

    function addSamePersonCertificate() {
      ...
      form.id = `marriageCertForm${certCountNo}`;
      form.querySelector("h3").textContent = `Certificate #${certCountNo}`;

      // Copy values from first certificate into newCert (excluding requester info)
      const firstHusbandFields = firstCert.querySelectorAll(".mb-4:nth-of-type(3) input");
      const firstWifeFields = firstCert.querySelectorAll(".mb-4:nth-of-type(4) input");
      const firstMarriageFields = firstCert.querySelectorAll(".mb-4:nth-of-type(5) input");
      
      const newHusbandFields = newCert.querySelectorAll(".mb-4:nth-of-type(3) input");
      const newWifeFields = newCert.querySelectorAll(".mb-4:nth-of-type(4) input");
      const newMarriageFields = newCert.querySelectorAll(".mb-4:nth-of-type(5) input");

      // Copy husband info
      firstHusbandFields.forEach((input, i) => {
        if (input.type !== "button" && input.type !== "submit") {
          newHusbandFields[i].value = input.value;
        }
      });
      
      // Copy wife info
      firstWifeFields.forEach((input, i) => {
        if (input.type !== "button" && input.type !== "submit") {
          newWifeFields[i].value = input.value;
        }
      });
      
      // Copy marriage details
      firstMarriageFields.forEach((input, i) => {
        if (input.type !== "button" && input.type !== "submit") {
          newMarriageFields[i].value = input.value;
        }
      });
    ...
    function addDifferentPersonCertificate() {
      const container = document.querySelector("main");
      const allCerts = document.querySelectorAll(".form-box");
      const lastCert = allCerts[allCerts.length - 1];
      ...
      // Clear all inputs in the new certificate except requester info
      const inputs = newCert.querySelectorAll(".mb-4:not(:first-child) input, .mb-4:not(:first-child) select");
      ...
      // Preserve requester info (first section)
      const firstCert = allCerts[0];
      const firstRequesterFields = firstCert.querySelectorAll(".mb-4:first-of-type input, .mb-4:first-of-type select");
      const newRequesterFields = newCert.querySelectorAll(".mb-4:first-of-type input, .mb-4:first-of-type select");
      
      firstRequesterFields.forEach((input, i) => {
        if (input.type !== "button" && input.type !== "submit" && input.type !== "file") {
          newRequesterFields[i].value = input.value;
        }
      });
