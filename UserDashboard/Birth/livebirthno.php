...
<head>
  ...
  <title>Live Birth Certificate - Not Requesting for Self</title>
...
<body>
  ...
  <main class="container">
    ...
    <!-- Initial Certificate -->
    <div class="form-box">
      <form class="birth-form" id="birthCertForm">
        <h3>Certificate #1</h3>
        ...
        <div class="row g-3">
          ...
          <div class="col-md-3"><label class="form-label required">Relationship to the Child</label>
        ...
        <hr class="my-4" />
        <h4>Person/Child Information</h4>
        <div class="row g-3">
          <div class="col-md-4"><label class="form-label required">First Name</label><input type="text" class="form-control" required /></div>
          <div class="col-md-4"><label class="form-label">Middle Name</label><input type="text" class="form-control" /></div>
          <div class="col-md-4"><label class="form-label required">Last Name</label><input type="text" class="form-control" required /></div>
        ...
        <hr class="my-4" />
        <h4>Complete Name of the Father</h4>
        <div class="row g-3">
          <div class="col-md-4"><label class="form-label required">First Name</label><input type="text" class="form-control" required /></div>
          <div class="col-md-4"><label class="form-label">Middle Name</label><input type="text" class="form-control" /></div>
          <div class="col-md-4"><label class="form-label required">Last Name</label><input type="text" class="form-control" required /></div>
        ...
        <h4 class="mt-3">Mother's Maiden Name</h4>
        <div class="row g-3">
          <div class="col-md-4"><label class="form-label required">First Name</label><input type="text" class="form-control" required /></div>
          <div class="col-md-4"><label class="form-label">Middle Name</label><input type="text" class="form-control" /></div>
          <div class="col-md-4"><label class="form-label required">Last Name</label><input type="text" class="form-control" required /></div>
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <div class="alert alert-warning" id="checklistWarning" style="display: none;">
            <strong>Warning:</strong> You must select at least one ID and check the Authorization Letter to proceed.
          </div>
          <div class="mb-3">
          ...
          <div class="checklist-item mt-3">
            <input class="form-check-input" type="checkbox" id="authLetter">
            <label class="form-check-label" for="authLetter">Authorization Letter</label>
          </div>
  ...
  <script>
    ...
    function validateAndShowChecklist() {
      ...
      if (form.checkValidity()) {
        ...
        document.getElementById('authLetter').checked = false;
    ...
    function proceedToNextStep() {
      ...
      // Double-check if authorization letter is checked
      const authLetterChecked = document.getElementById('authLetter').checked;
      ...
      if (anyIdSelected && authLetterChecked) {
        ...
        if (!authLetterChecked) {
          // Highlight authorization letter label
          const authLabel = document.querySelector('label[for="authLetter"]');
          authLabel.classList.add('text-danger', 'fw-bold');
          setTimeout(() => {
            authLabel.classList.remove('text-danger', 'fw-bold');
          }, 3000);
        }
      }
    }
    ...
    function handleYes() {
      ...
      modal.hide();
      createNoSelfFields(true); // YES = remove requester info
    }

    function handleNo() {
      const modal = bootstrap.Modal.getInstance(document.getElementById("confirmModal"));
      modal.hide();
      createNoSelfFields(false); // NO = clone entire form, including requester info
    }

    function createNoSelfFields(cloneOnlyChildSection = false) {
      const container = document.querySelector("main");
      const allCerts = document.querySelectorAll(".form-box");
      const lastCert = allCerts[allCerts.length - 1];

      // Remove buttons from previous certificate
      const oldBtns = lastCert.querySelector(".d-flex.justify-content-between");
      if (oldBtns) oldBtns.remove();

      const wrapper = document.createElement("div");
      wrapper.classList.add("form-box", "mt-4");

      const firstForm = document.querySelector(".form-box form");

      const newForm = document.createElement("form");
      newForm.className = "birth-form";
      newForm.innerHTML = `<h3>Certificate #${++certCount}</h3>`;

      if (cloneOnlyChildSection) {
        const headersToCopy = [
        ...
        headersToCopy.forEach(headerText => {
          const header = Array.from(firstForm.querySelectorAll("h4"))
            .find(h => h.textContent.trim() === headerText);
          if (header) {
            let node = header;
            while (node && (node.tagName !== "H3")) {
              newForm.appendChild(node.cloneNode(true));
        ...
        // Full clone including values
        const clonedHTML = firstForm.innerHTML.replace(/<h3>Certificate #\d+<\/h3>/, "");
        newForm.innerHTML += clonedHTML;

        const firstInputs = firstForm.querySelectorAll("input, select");
      ...
      const prevRemoveBtn = newForm.querySelector(".btn-danger");
      if (prevRemoveBtn) prevRemoveBtn.remove();

      // Update the Next button to use validateAndShowChecklist
